...
# silver_supplier_sku_map.sku_id corresponds to source_product_id where
# source_system = "supplier_internal" in the crosswalk.
# NOTE: confirm "supplier_internal" is the actual literal value in your data:
#   crosswalk.select("source_system").distinct().show()
canonical_map = crosswalk.filter(
    F.col("source_system") == "supplier_internal"
).select(
    F.col("source_product_id").alias("sku_id"),
    F.col("canonical_sku_id")
).dropDuplicates(["sku_id"])

# If sku_id already IS the canonical (e.g. the self-mapping rows added during
# Silver relationship validation for the 200/198 missing SKU IDs), the
# coalesce below falls back to using sku_id directly as canonical_sku_id.
...
best_alternate_renamed = best_alternate.select(
    F.col("sku_id").alias("_alt_sku_id"),
    F.col("primary_supplier_id").alias("_alt_primary_supplier_id"),
...
dependency_mart = (
    ...
    .join(
        best_alternate_renamed,
        on=[
            sku_map_resolved["sku_id"] == best_alternate_renamed["_alt_sku_id"],
            sku_map_resolved["supplier_id"] == best_alternate_renamed["_alt_primary_supplier_id"]
        ],
        how="left"
    )
    .select(
        sku_map_resolved["supplier_id"],
        F.col("supplier_name"),
        F.col("country"),
        F.col("criticality_tier"),
        F.col("single_source_flag"),
        sku_map_resolved["sku_id"],
        sku_map_resolved["canonical_sku_id"],
        sku_map_resolved["dependency_percent"],
        sku_map_resolved["is_primary_supplier"],
        sku_map_resolved["standard_lead_time_days"].alias("std_lead_time_days"),
        sku_map_resolved["origin_country"],
        sku_map_resolved["origin_port"],
        sku_map_resolved["destination_dc"],
        sku_map_resolved["transport_mode"],
        sku_map_resolved["minimum_order_quantity"],
        best_alternate_renamed["best_alternate_supplier_id"],
        F.coalesce(best_alternate_renamed["alternate_status"], F.lit("none")).alias("alternate_status"),
        best_alternate_renamed["switching_cost_level"],
        best_alternate_renamed["estimated_switch_days"],
        best_alternate_renamed["capacity_available_pct"],
