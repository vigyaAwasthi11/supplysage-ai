...
gold_dim_suppliers = supplier_network.select(
    ...
    F.col("avg_standard_lead_time_days").alias("avg_standard_lead_time"),
    F.col("avg_standard_transit_days"),
    F.col("purchase_order_count").alias("po_count"),
    F.col("purchase_order_line_count").alias("po_line_count"),
    F.col("avg_po_fill_rate"),
    F.col("avg_actual_lead_time_days").alias("avg_actual_lead_time"),
    F.col("avg_delivery_delay_days").alias("avg_delivery_delay"),
    F.col("late_po_line_count"),
    F.col("latest_scorecard_month"),
    F.col("latest_fill_rate"),
    F.col("latest_on_time_delivery_rate"),
    F.col("latest_quality_issue_rate"),
    F.col("latest_avg_lead_time_days"),
    F.col("latest_lead_time_variance"),
    F.col("latest_defect_rate"),
    F.col("latest_is_deteriorating").alias("deterioration_flag"),
    F.col("route_count"),
